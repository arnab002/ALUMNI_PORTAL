...
const EditBook = ({ book, onClose }) => {
    ...
    useEffect(() => {
        const fetchBookDetails = async () => {
          try {
            const response = await axios.get(`${baseApiURL()}/getSingleBookDetails/${book._id}`);
    ...
    useEffect(() => {
        if (isDataChanged && bookData.bookimg) {
          ...
          axios.post(`${baseApiURL()}/editBookDetails/${book._id}`, formData, {
            headers: {
            ...
            .catch(error => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    text: 'Error Updating Book Details',
                    confirmButtonText: 'OK'
                  });
            })
            .finally(() => {
      ...
      }, [isDataChanged, bookData, book, bookData.bookimg]);
                                ...
                                <Row>
                                    <Col className='d-flex justify-content-center align-items-center'>
                                        <FormGroup className='d-flex justify-content-center align-items-center border border-black' style={{ width: '100%', height: '100%', borderRadius: '10px' }}>
                                            <img src={bookData.bookimg} width={150} height={150} style={{ borderRadius: '10px' }} alt="Existing Student Image Will Load Here" />
                                        </FormGroup>
                                    </Col>
                                    <Col className='d-flex justify-content-center align-items-center'>
                                        <FormGroup className='d-flex justify-content-center align-items-center border border-black' style={{ width: '100%', height: '100%', borderRadius: '10px' }}>
