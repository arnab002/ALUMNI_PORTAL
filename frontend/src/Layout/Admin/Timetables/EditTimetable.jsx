...
const EditTimetable = ({timetable , onClose}) => {
  ...
  useEffect(() => {
    const fetchTimetableDetails = async () => {
      try {
        const response = await axios.get(`${baseApiURL()}/getSingleTimetableDetails/${timetable._id}`);
        const TimetableDetails = response.data;

        setTimetableData(TimetableDetails);
      } catch (error) {
          Swal.fire({
            icon: 'error',
            title: 'Error!',
            text: 'Error Fetching Timetable!',
            confirmButtonText: 'OK'
          });
      }
    };
  ...
  useEffect(() => {
    if (isDataChanged && timetableData.file) {
      ...
      axios.post(`${baseApiURL()}/editTimetableDetails/${timetable._id}`, formData, {
        headers: {
  ...
  }, [isDataChanged, timetableData, timetable, timetableData.file]);
